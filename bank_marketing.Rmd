...
most of the data is missing in "nr.employed" and because it has only one unique value, we are going to drop the whole column 

```{r}
bank <- select(bank, - nr.employed)
```
...
```{r}
bank <- bank %>% mutate_if(is.ordered, factor, ordered = FALSE)
```
...
```{r}
set.seed(123) # for reproducibility
split_understand <- initial_split(bank, strata = "y", prop = 0.2)
train_understand <- training(split_understand)
...
```{r}
set.seed(123) # for reproducibility
split_df <- initial_split(df_bank, strata = "y", prop = 0.7)
...
because "knn" can not handle categorical features I got this error 
```{r}
knn_fit <- train(
  y ~ ., 
  data = train, 
...
bank <- bank %>% 
  select(-duration, -nr.employed) %>% 
  mutate(y = as_factor(y))

names(bank[,17:19])

set.seed(123) # for reproducibility
split <- initial_split(bank[,17:19], strata = "y", prop = 0.00625)
train <- training(split)
...
Now reapply the model to your data that has feature engineered.(blueprint)
```{r}
Bp_model <- train(
  blueprint, 
  data = train, 
